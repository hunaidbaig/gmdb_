...
class GmdbApiApplicationTests {
    ...
    public void UpdateReview() throws Exception {
        // Create a reviewer and review
        Reviewer reviewer = new Reviewer(1, "John");
        Review review = new Review(1, reviewer, "Old review text");

        // Mock the reviewer and review repositories
        when(reviewerRepository.findById(1L)).thenReturn(Optional.of(reviewer));
        when(reviewRepository.findById(1L)).thenReturn(Optional.of(review));
        when(reviewRepository.save(any(Review.class))).thenReturn(review);

        // Define the updated review text
        String updatedReviewText = "Updated review text";

        // Create the request body
        ReviewRequest request = new ReviewRequest(updatedReviewText);

        // Perform the update review request
        mockMvc.perform(put("/reviewers/1/reviews/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(new ObjectMapper().writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(content().string("Review updated successfully"));

        // Verify that the review text was updated
        assertThat(review.getReviewText()).isEqualTo(updatedReviewText);
    }
}
